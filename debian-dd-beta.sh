...
ROOT_SOURCE=$(findmnt -fn -o SOURCE /)
# Strip any Btrfs subvolume suffix like /dev/vda3[/@rootfs]
ROOT_DEVICE="${ROOT_SOURCE%%\[*}"
case "$ROOT_DEVICE" in
    UUID=*)
        ROOT_BLK=$(blkid -U "${ROOT_DEVICE#UUID=}" 2>/dev/null)
        ;;
    LABEL=*)
        ROOT_BLK=$(blkid -L "${ROOT_DEVICE#LABEL=}" 2>/dev/null)
        ;;
    *)
        ROOT_BLK="$ROOT_DEVICE"
        ;;
esac
ROOT_BLK=$(readlink -f "$ROOT_BLK" 2>/dev/null || echo "$ROOT_BLK")
# Fall back to df output if detection failed
if [ -z "$ROOT_BLK" ] || [ ! -b "$ROOT_BLK" ]; then
    ROOT_BLK=$(df / | awk 'NR==2 {print $1}')
fi
...
if [ -n "$ROOT_BLK" ] && [ -b "$ROOT_BLK" ]; then
    ...
    if [ -z "$PARENT_DISK" ]; then
        ...
        PARENT_DISK=$(lsblk -no name "$ROOT_BLK" 2>/dev/null | head -n1)
    fi
fi
# If still empty, fallback to parsing df output
if [ -z "$PARENT_DISK" ]; then
    ROOT_DF=$(df / | awk 'NR==2 {print $1}')
    PARENT_DISK=$(lsblk -no pkname "$ROOT_DF" 2>/dev/null | head -n1)
    if [ -z "$PARENT_DISK" ]; then
        PARENT_DISK=$(lsblk -no name "$ROOT_DF" 2>/dev/null | head -n1)
    fi
fi
if [ -z "$PARENT_DISK" ]; then
...
rm -rf temp_initrd 
# Determine GRUB drive specification
GRUB_PART_PREFIX=""
if command -v lsblk >/dev/null 2>&1; then
    PART_TABLE=$(lsblk -no PTTYPE "/dev/$DEVICE_PREFIX" 2>/dev/null | head -n1)
    case "$PART_TABLE" in
        gpt)
            GRUB_PART_PREFIX="gpt"
            ;;
        dos|msdos)
            GRUB_PART_PREFIX="msdos"
            ;;
    esac
fi
if [ -n "$GRUB_PART_PREFIX" ]; then
    GRUB_DRIVE_SPEC="(hd0,${GRUB_PART_PREFIX}${partitionr_root_number})"
else
    GRUB_DRIVE_SPEC="(hd0,$partitionr_root_number)"
fi
if command -v grub-probe >/dev/null 2>&1; then
    if GRUB_PROBED_DRIVE=$(grub-probe --target=drive / 2>/dev/null | head -n1); then
        if [ -n "$GRUB_PROBED_DRIVE" ] && [[ "$GRUB_PROBED_DRIVE" != *hostdisk* ]]; then
            GRUB_DRIVE_SPEC="$GRUB_PROBED_DRIVE"
        fi
    fi
fi

# Resolve the GRUB-visible path for downloaded assets (handles Btrfs subvolumes)
GRUB_LINUX_PATH="/netboot/linux"
GRUB_INITRD_PATH="/netboot/initrd.gz"
if command -v grub-mkrelpath >/dev/null 2>&1; then
    if PROBED_LINUX=$(grub-mkrelpath "$GRUB_LINUX_PATH" 2>/dev/null); then
        [ -n "$PROBED_LINUX" ] && GRUB_LINUX_PATH="$PROBED_LINUX"
    fi
    if PROBED_INITRD=$(grub-mkrelpath "$GRUB_INITRD_PATH" 2>/dev/null); then
        [ -n "$PROBED_INITRD" ] && GRUB_INITRD_PATH="$PROBED_INITRD"
    fi
fi

cat << EOF >> /etc/grub.d/40_custom
menuentry "DigVPS.COM Debian Installer AMD64" {
    set root="$GRUB_DRIVE_SPEC"
    linux $GRUB_LINUX_PATH auto=true priority=critical lowmem/low=true preseed/file=/preseed.cfg
...
echo "  Boot partition : ${GRUB_DRIVE_SPEC} in GRUB entry"
